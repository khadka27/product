...
import { updateProduct } from "@/lib/models";
...
export async function PUT(
  ...
  try {
    ...
    // Process text fields
    const textFields = [
      'old_name', 'new_name', 'description', 'next_redirect_url', 
    ...
    let description_points = [];
    try {
      const description_points_json = formData.get('description_points') as string;
      if (description_points_json) {
        description_points = JSON.parse(description_points_json);
      } else {
        // Try to collect individual points
